---
title: "Production Performance Management Protocol Use Cases: Simple Transformation with Apache Camel"
...
# The Approach
We want to find a way to receive the data, transform it into Production Performance Management Protocol and store it into a database in our [REST Server](https://unide.eclipse.org).
...
But wait! The welding payload above was not Production Performance Management Protocol. So we also need to normalize the payload.
...
2. transform the PSI 6000 java object to two Production Performance Management Protocol-java objects ([measurement-message](/unide/specification/measurement-message) and [process-message](https://www.eclipse.org/unide/specification/process-message))
3. split up the two Production Performance Management Protocol objects
4. route the objects individually
5. transform each Production Performance Management Protocol object back to a String
...
### Use unide.java binding for Production Performance Management Protocol Java Objects
For eventually transforming the Production Performance Management Protocol objects back to String ("*marshal*"), we can make use of the [unide Production Performance Management Protocol java binding](https://github.com/eclipse/unide.java). Just adjust the dataFormats section of the Spring XML so that it reads:
```xml
...
<camelContext xmlns="http://camel.apache.org/schema/spring">
	<dataFormats>
...
###  Transforming from PSI 6000 POJO to Production Performance Management Protocol POJOs
...
Apache Camel offers a ``Processor`` interface that can be used to override the ``process()`` function. This function receives the complete message (``exchange``) and can modify it in any way. Since we have created the PSI 6000 Java Object already and want to produce Production Performance Management Protocol java objects, the logic is mostly about creating the child objects and wiring the suitable getter to corresponding setter.
