...
public class RestEndpointPostgresTest {
   ...
   public void setUp( TestContext context ) throws IOException, ReceiverException, SQLException {
      ...
      Mockito.when( dsMock.getConnection() )
      ...
      Mockito.when( dependencyProvider.getConnection() )
             .thenReturn( () -> JdbcMockFactory.getMockConnection( WIREMOCK_URL + "/sqlStub" ) );
      Mockito.when( dependencyProvider.getDataSource() )
      		 .thenReturn( dsMock );
   ...
   private JsonObject createConfig() {
      return new JsonObject()
            .put( "http.port", PORT )
            .put( "sqlDb.url", "jdbc:postgresql://localhost:5432/unide_ppmp" )
            .put("sqlDb.user", "postgres")
            .put("sqlDb.password", "")
            .put("persistence.enable", true)
